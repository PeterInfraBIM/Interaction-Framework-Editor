		...
		private class MessageItem {
			...
			public MessageItem(final Canvas canvas, Graphics g, final MessageInTransactionTypeType mitt, int x, int y) {
				...
				activeLabel.addMouseListener(new MouseAdapter() {
					...
					@Override
					public void mouseClicked(MouseEvent e) {
						Editor14.getMainFrameControl().navigate(getMessage(mitt));
					}
				});
...
//				}
	...
	private void initResponsibilityFeedback() {
		tfd_ResponsibilityFeedback.getDocument().addDocumentListener(new DocumentAdapter14() {
			@Override
	...
	private void initResponsibilitySupportTask() {
		tfd_ResponsibilitySupportTask.getDocument().addDocumentListener(new DocumentAdapter14() {
			@Override
			protected synchronized void update(DocumentEvent e) {
				if (inSelection)
					return;
	...
	private void initResponsibilityTask() {
		tfd_ResponsibilityTask.getDocument().addDocumentListener(new DocumentAdapter14() {
			@Override
			protected synchronized void update(DocumentEvent e) {
				if (inSelection)
					return;
	...
	private void initResponsibilityScope() {
		tfd_ResponsibilityScope.getDocument().addDocumentListener(new DocumentAdapter14() {
			@Override
			protected synchronized void update(DocumentEvent e) {
				if (inSelection)
					return;
	...
	private void initTransactionsTable() {
		...
		navigateColumn.setCellEditor(new NavigatorEditor() {
			...
			protected void navigate() {
				...
				Editor14.getMainFrameControl().navigate(transactionsTableModel.get(row));
			}
		});
	}
	...
	private void initMessagesTable() {
		...
		navigateColumn.setCellEditor(new NavigatorEditor() {
			...
			protected void navigate() {
				...
				Editor14.getMainFrameControl().navigate(message);
			}
		});
