...
public class ComplexElementTreeTransferHandler<E extends ElementType> extends TransferHandler {
	private final PanelControl14<E> panelControl;
	...
	public ComplexElementTreeTransferHandler(PanelControl14<E> panelControl, JTree tree) {
		super();
	...
	public boolean canImport(TransferSupport support) {
		if (support.isDataFlavorSupported(DataFlavor.stringFlavor)) {
			try {
				...
				simpleElement = Editor14.getStore14().getElement(SimpleElementTypeType.class, id);
				if (simpleElement != null) {
					return canImport(support, simpleElement);
				} else {
					complexElement = Editor14.getStore14().getElement(ComplexElementTypeType.class, id);
			...
			if (movedNode != null) {
				...
				if (!(dropNode.getUserObject() instanceof ComplexElementTypeType)
					...
					if (parentNode.getUserObject() instanceof ComplexElementTypeType && dropNode.equals(parentNode)
						...
						List<ComplexElementTypeType> complexElements = Control14
	...
	private boolean canImport(TransferSupport support, SimpleElementTypeType simpleElement) {
		...
		if (path != null) {
			...
			if (dropNode.getUserObject() instanceof ComplexElementTypeType) {
				...
				List<ComplexElementTypeType> complexElements = Control14.getComplexElements(ce);
				List<SimpleElementTypeType> simpleElements = Control14.getSimpleElements(ce);
		...
		if (dropLocation.getPath() == null) {
			...
			if (dropNode.getUserObject() instanceof ComplexElementTypeType) {
				...
				if (simpleElement != null && dropLocation.getChildIndex() > -1) {
					...
					List<ComplexElementTypeType> complexElements = Control14.getComplexElements(dropElement);
					if (complexElements == null || complexElements.isEmpty()) {
					...
					List<SimpleElementTypeType> simpleList = Control14.getSimpleElements(complexElement);
					if (simpleList != null) {
						int index = 0;
	...
	void showComplexNode(DefaultMutableTreeNode complexNode) {
		...
		List<ComplexElementTypeType> complexList = Control14.getComplexElements(complexElement);
		if (complexList != null) {
			for (ComplexElementTypeType ce : complexList) {
				...
				List<SimpleElementTypeType> seList = Control14.getSimpleElements(ce);
				if (seList != null) {
					int index2 = 0;
