...
import nl.visi.interaction_schema.TransactionTypeType.SubTransactions;
import nl.visi.interaction_schema.TransactionTypeTypeRef;

public class TransactionsPanelControl12 extends PanelControl12<TransactionTypeType> {
	private static final String TRANSACTIONS_PANEL = "nl/visi/interaction_framework/editor/swixml/TransactionsPanel12.xml";
	...
	private JTextField tfd_Result, tfd_BasePoint;
	...
	public class Canvas extends JPanel {
		...
		private class MessageItem {
			...
			public MessageItem(MessageInTransactionTypeType mitt) {
				...
				this.loop = false;
				this.incomingConnections = new ArrayList<TransactionsPanelControl12.Canvas.MessageItem>();
				this.outgoingConnections = new ArrayList<TransactionsPanelControl12.Canvas.MessageItem>();
			}
		...
		public Canvas() {
			...
			messages = new ArrayList<TransactionsPanelControl12.Canvas.MessageItem>();
		}
				...
				for (int index = 0; index < messagesTableModel.getRowCount(); index++) {
					...
					if (message != null) {
						...
						if (messageType != null) {
							...
							messages.add(item);
							item.setStartMitt(TransactionsPanelControl12.this.isStart(mitt, selectedElement));
	...
	private ListSelectionListener messageTableSelectionListener = new ListSelectionListener() {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if (selectedMessage) {
				...
				if (roleType != null) {
					List<MessageInTransactionTypeType> mitts = Editor12.getStore12()
		...
		private void fillElementConditionsTable(MessageInTransactionTypeType mitt) {
			elementConditionsTableModel.clear();
			List<ElementConditionType> elements = Editor12.getStore12().getElements(ElementConditionType.class);
	...
	public class MessagesTableModel extends ElementsTableModel<MessageInTransactionTypeType> {
		...
		private void setTransactionPhase(Object value, MessageInTransactionTypeType mitt) {
			TransactionPhaseTypeType transactionPhaseType = value != null
					? Editor12.getStore12().getElement(TransactionPhaseTypeType.class, (String) value)
					: null;
		...
		private void setGroup(Object value, MessageInTransactionTypeType mitt) {
			GroupTypeType groupType = value != null
					? Editor12.getStore12().getElement(GroupTypeType.class, (String) value)
					: null;
		...
		private List<MittNode> getMittNodes(List<MittNode> mittNodes, MittNode parent,
			...
			if (mittNodes == null) {
				mittNodes = new ArrayList<TransactionsPanelControl12.MessagesTableModel.MittNode>();
			}
		...
		public void setValueAt(Object value, int rowIndex, int columnIndex) {
			...
			case ComplexElement:
				if (value == null) {
					...
					ComplexElementTypeType ce = Editor12.getStore12().getElement(ComplexElementTypeType.class, idref);
			...
			case SimpleElement:
				if (value == null) {
					...
					String idref = (String) value;
					SimpleElementTypeType se = Editor12.getStore12().getElement(SimpleElementTypeType.class, idref);
	...
	public TransactionsPanelControl12() throws Exception {
	...
	private void initBasePointField() {
		tfd_BasePoint.getDocument().addDocumentListener(new DocumentAdapter12() {
			@Override
	...
	private void initResultField() {
		tfd_Result.getDocument().addDocumentListener(new DocumentAdapter12() {
			@Override
			protected synchronized void update(DocumentEvent e) {
				if (inSelection)
					return;
	...
	private void initPreviousMessagesTable() {
		...
		navigateColumn.setCellEditor(new NavigatorEditor() {
			...
			protected void navigate() {
				...
				if (messageType != null) {
					Editor12.getMainFrameControl().navigate(messageType);
				}
			}
		});
	}
	...
	protected void updateSelectionArea(ListSelectionEvent e) {
		...
		if (rowSelected) {
			...
			List<RoleTypeType> elements = Editor12.getStore12().getElements(RoleTypeType.class);
			...
			List<TransactionPhaseTypeType> phasesList = Editor12.getStore12()
			...
			List<GroupTypeType> groupsList = Editor12.getStore12().getElements(GroupTypeType.class);
			...
			List<MessageTypeType> messages = Editor12.getStore12().getElements(MessageTypeType.class);
	...
	private void fillSubtransactionsTable() {
		...
		List<Object> mittList = Editor12.getStore12().getElements(MessageInTransactionTypeType.class);
	...
	public void setInitiator() {
		...
		if (itemId != null) {
			RoleTypeType roleType = Editor12.getStore12().getElement(RoleTypeType.class, itemId);
	...
	public void removeElementCondition() {
		...
		Editor12.getStore12().remove(elementConditionType.getId());
	...
	public void navigateInitiator() {
		...
		RoleTypeType element = Editor12.getStore12().getElement(RoleTypeType.class, idref);
	...
	private void initPrevMap() {
		List<Object> mittObjects = Editor12.getStore12().getElements(MessageInTransactionTypeType.class);
