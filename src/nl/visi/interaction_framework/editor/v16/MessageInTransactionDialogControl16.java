	...
	public void removeElementCondition() {
		...
		if (userObject != null) {
			...
			if (confirm == JOptionPane.OK_OPTION) {
				if (userObject instanceof SimpleElementTreeNode) {
					SimpleElementTreeNode seNode = (SimpleElementTreeNode) userObject;
					ElementConditionType ec = getElementConditionType(seNode.getMitt(), seNode.getPce(),
							seNode.getCce(), seNode.getSe());
		...
		selectBox.addPropertyChangeListener(new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				...
				MessageInTransactionTypeType sendBeforeMitt = Editor16.getStore16()
						.getElement(MessageInTransactionTypeType.class, mittId);
				Control16.addSendBefore(currentMitt, sendBeforeMitt);
		...
		selectBox.addPropertyChangeListener(new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				...
				MessageInTransactionTypeType sendAfterMitt = Editor16.getStore16()
						.getElement(MessageInTransactionTypeType.class, mittId);
				Control16.addSendAfter(currentMitt, sendAfterMitt);
